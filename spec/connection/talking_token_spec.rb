shared_context "talking token:" do
    ...
    context "Service::Connection::TalkingToken: is the class representing obtained talking tokens from the
      ...
      context "- Instantiation:" do
        ...
        context 'params presence:' do
          before(:each) do
            allow(authorizator_client).to receive(:talking_token).and_return(valid_talking_token_data)
            allow(Authorizator::Client).to receive(:new).and_return(authorizator_client)
        ...
        context "at instantiation time, a new talking token data from Authorizator service is obtained, so:" do
          ...
          also_raise "a talking token error if", "no remote talking token can be obtained from the Authorizator service." do
            before {allow(authorizator_client).to receive(:talking_token).and_return(no_talking_token_data)}
          end

          also_raise "a talking token error if", "a void remote talking token is received." do
            before {allow(authorizator_client).to receive(:talking_token).and_return(empty_talking_token_data)}
          end

          also_raise "a talking token error if", "a no Hash remote talking token is received." do
            before {allow(authorizator_client).to receive(:talking_token).and_return(no_hash_talking_token_data)}
          end

          also_raise "a talking token error if", "a remote talking token with no token string is received." do
            before {allow(authorizator_client).to receive(:talking_token).and_return(no_token_value_talking_token_data)}
          end

          also_raise "a talking token error if", "a remote talking token with empty token string is received." do
            before {allow(authorizator_client).to receive(:talking_token).and_return(empty_token_value_talking_token_data)}
          end

          also_raise "a talking token error if", "a remote talking token with invalid scope is received." do
            before {allow(authorizator_client).to receive(:talking_token).and_return(invalid_scope_talking_token_data)}
          end
        end
