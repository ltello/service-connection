shared_context "response:" do
  ...
  describe 'The response of the Called service to a request to access its api, differs depending on the request,
    ...
    let(:invalid_talking_token_header_response)     {double(:error => double(:code => 7), :headers => {'www-authenticate' => (random_string_1 + invalid_talking_token_error_codes.first + random_string_2)})}
    ...
    context '#invalid_talking_token_response?(resp):' do
      ...
      it 'If it does, checks the response headers looking for an error condition (in www-authenticate header right now).' do
        expect(invalid_talking_token_error_codes).to include(service_connection.send(:invalid_talking_token_response?, invalid_talking_token_header_response))
      end

      it 'returns false when the response do not respond to #error...' do
        expect(service_connection.send(:invalid_talking_token_response?, no_error_responding_response)).to be(false)
      end

      it '...or the error object is present.' do
        expect(service_connection.send(:invalid_talking_token_response?, error_blank_response)).to be(false)
      end
      ...
      it 'In any other case, invalid_talking_token_response? should be false' do
        expect(service_connection.send(:invalid_talking_token_response?, no_invalid_talking_token_error_response)).to be(false)
      end
    end
